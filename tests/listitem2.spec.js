...
test('List Demo', async ({ page }) => {
  ...
  const listItems = listSection.locator('li');//also can be done by getByRole

  
  // Assert that there are 4 list items
  await expect(listItems).toHaveCount(4);

  //Assert all the elements in the list
  await expect(listItems).toHaveText(['Access all currently published courses','Access to any new published courses till you are paid member','Per month price ','Cancel anytime'])
});